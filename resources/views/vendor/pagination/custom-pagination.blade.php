    ...
    <nav role="navigation" aria-label="Pagination Navigation" class="flex flex-col items-center gap-3">
        <div class="flex items-center space-x-1">
            ...
            <a href="{{ $paginator->previousPageUrl() }}" @class([
                ...
                'bg-green-100 text-green-700 hover:bg-green-200' => !$paginator->onFirstPage(),
                'bg-green-50 text-green-400 cursor-not-allowed' => $paginator->onFirstPage(),
            ])
            ...
            @foreach ($elements as $element)
                @if (is_string($element))
                    <span class="px-3 py-2 bg-green-50 text-green-700 border">{{ $element }}</span>
                @endif
            ...
            <a href="{{ $paginator->nextPageUrl() }}" @class([
                ...
                'bg-green-100 text-green-700 hover:bg-green-200' => $paginator->hasMorePages(),
                'bg-green-50 text-green-400 cursor-not-allowed' => !$paginator->hasMorePages(),
            ])
